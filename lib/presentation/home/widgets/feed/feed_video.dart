...
class _FeedVideoState extends State<FeedVideo> {
  ...
  void _initializeVideoController() {
    ...
    if (Utils.isLocalFilePath(widget.videoUrl)) {
      videoPath = Utils.getActualFilePath(widget.videoUrl);
      print('Loading local video: $videoPath');

      // Validate that the file path is not empty
      if (videoPath.isEmpty) {
        print(
          'Error: Empty video path after processing videoUrl: ${widget.videoUrl}',
        );
        setState(() {
          _hasError = true;
        });
